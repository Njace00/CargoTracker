...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO reservations (account_id, reservation_date, company_name, shipment, address_destination, status, created_at) 
            VALUES (?, ?, ?, ?, ?, 'pending', NOW())";
    
    $stmt = $conn->prepare($sql);
    
    if ($stmt) {
        $stmt->bind_param("issss", $account_id, $reservation_date, $company_name, $shipment, $address_destination);
